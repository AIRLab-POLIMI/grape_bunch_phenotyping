...
def do_test(cfg, model):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        if comm.is_main_process():
            ...
            # run['metrics/AP_segm_test'].log(results_i['segm']['AP'])
    
    return results[dataset_name]['segm']['AP']
...
def objective(trial, cfg, args):
    ...
    # cfg.SOLVER.BASE_LR = trial.suggest_float("base_lr", 5e-3, 1e-1, log=True)  # If log is true, the value is sampled from the range in the log domain
    ...
    # cfg.SOLVER.GAMMA = trial.suggest_float("gamma", 0.1, 0.9, step=0.1) 
...
def main(args):
    ...
    study = optuna.create_study(direction="maximize", study_name="lr_freeze_detailed", storage='sqlite:///optuna-db/lr_freeze_detailed.db')
    study.optimize(functools.partial(objective, cfg=cfg, args=args),  # I use functools to create a new function with the additional arguments
                   n_trials=15,               # The number of trials for each process
